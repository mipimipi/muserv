		...
		if strings.HasPrefix(path, "http://") || strings.HasPrefix(path, "https://") {
			...
			if !exists {
				if t, err = newExtTrack(cnt, count, path, title); err != nil {
					log.Error(errors.Wrapf(err, "cannot create a track for playlist item '%s': ignore it", path))
					continue
				}
				if len(item.Title) > 0 {
					title = p.Base(file.PathTrunk(uri.Path))
				}
				t.n = title
				t.sf = []string{title}
			}
			...
			if exists, err = file.Exists(path); err != nil {
				log.Error(errors.Wrapf(err, "cannot check existence of playlist item '%s': ignore it", path))
				continue
			}
			if !exists {
				log.Errorf("playlist item '%s' doesn't exist: ignore it", path)
				continue
			}
